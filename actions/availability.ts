...
export async function getUserAvailability() {
  ...
  availabilityData.timeGap = user.availability.timeGap;
...
interface AvailabilityData {
  timeGap: number; // Ensure timeGap is properly typed
  [key: string]: { 
    isAvailable: boolean; 
    startTime: string; 
    endTime: string;
...
export async function updateAvailability(data: AvailabilityData) {
  ...
  const availabilityData = Object.entries(data).flatMap(([day, value]) => {
    if (day === "timeGap") return []; // Skip timeGap

    const { isAvailable, startTime, endTime } = value as { 
      isAvailable: boolean; 
      startTime: string; 
      endTime: string;
    };
  ...
  if (user.availability) {
    await db.availability.update({
      ...
      data: {
        timeGap: data.timeGap, // Ensure this is correctly typed as number
        days: {
          deleteMany: {},
...
function generateAvailableTimeSlots(
  ...
  while (currentTime < slotEndTime) {
    ...
    const isSlotAvailable = !bookings.some((booking: { startTime: Date; endTime: Date }) => {
